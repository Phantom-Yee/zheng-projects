...
def merge_and_shuffle_datasets(output_file):
    ...
    if len(all_data) >= sample_size:
        ...
        print(f"📋 已截取前 {sample_size} 条数据")
    else:
        print(f"⚠️  数据总量不足 {sample_size} 条，将使用全部 {len(all_data)} 条数据")
    ...
    print("📈 最终数据分布：", attack_count)

    # 6. 写入最终JSON文件
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(all_data, f, ensure_ascii=False, indent=2)
    print(f"\n✅ 最终文件已生成：{output_file}（共{len(all_data)}条数据）")


# ---------------------- 执行主函数 ----------------------
if __name__ == "__main__":
    # 输出文件路径（可自定义）
    OUTPUT_JSON = "merged_can_dataset2.json"
