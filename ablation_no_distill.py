...
【PyCharm 直接运行说明】
  1. 安装依赖（终端执行一次即可）：
     ...
     或让本脚本自动安装（首次运行时会自动调用 pip）。
  2. 直接点击 PyCharm 右上角绿色 ▶ Run 按钮即可。
  3. 数据文件：merged_can_dataset3.json（与本文件同目录）。
     若文件不存在，会自动生成 9000 条模拟数据供测试。
...
# ================================================================
# 0. 依赖自动安装（PyCharm 首次运行无需手动 pip install）
#    所有包都在这里统一处理，确保 import 不会因缺包而失败。
# ================================================================
def _ensure(pkg, import_as=None):
    """尝试 import，若不存在则自动 pip install（安装失败时会打印报错信息）。"""
    name = import_as or pkg.split(">=")[0].split("==")[0].replace("-", "_")
    try:
        __import__(name)
    except ImportError:
        print(f"⚙️  正在安装 {pkg}，请稍候...")
        # stderr 不重定向，安装失败时用户可见错误原因
...
# 核心依赖
_ensure("torch")
_ensure("torchvision")               # torch 生态，顺带装上
_ensure("transformers>=4.35.0", "transformers")
_ensure("scikit-learn",          "sklearn")
_ensure("numpy")
_ensure("accelerate>=1.1.0",     "accelerate")

# 可选量化依赖（GPU 路径需要；CPU 路径自动降级 INT8）
BNB_AVAILABLE = False
try:
...
# 2. 工具函数
# ================================================================
def make_fc(in_f: int, out_f: int) -> nn.Linear:
...
def balance_loss(gate_weights: torch.Tensor) -> torch.Tensor:
    """
    MoE 负载均衡损失：惩罚专家使用率不均匀。
    gate_weights: [B, 3]，已经过 softmax。
    强制 3 个专家被均等使用，防止退化成只用 1 个专家。
    """
...
def load_and_split(path: str = DATA_PATH):
    if not os.path.exists(path):
        print(f"⚠️  {path} 不存在，生成 9000 条模拟数据...")
        data  = []
        specs = [
...
class MoEStudentModel(nn.Module):
    ...
    这些 key 不在普通 BertModel 的 state_dict 里，直接调用
    model.load_state_dict(best_state) 就会触发 "Unexpected key(s)" 报错。

    解决方案：
      - get_saveable_state()：只提取 LoRA 参数 + MoE 头参数 的普通 tensor
      - load_saveable_state()：按名称精确更新对应参数，完全绕过 state_dict
      - BERT 量化结构（含 quant_state 等 buffer）始终不动，也不需要动
    """

    def __init__(self, bert_path: str, num_classes: int = 4, dropout: float = DROPOUT):
        super().__init__()
        self.bert_dim    = 768
        ...
        # ── 构建 QLoRA BERT ─────────────────────────────────
        print("\n  [QLoRA] 加载 12 层 BERT，应用 QLoRA...")
        self.bert = self._build_qlora_bert(bert_path)

        # ── 构建 MoE 头（与 dis2.py 完全一致）──────────────
        print("\n  [MoE]  构建 gate + 3 专家（结构与 dis2.py 一致）...")
        d = self.bert_dim
        ...
        print("  ✅ MoE 头: gate(768→128→3) + 3×专家(768→256→128→4)")

    # ── 构建 QLoRA BERT ──────────────────────────────────────
    def _build_qlora_bert(self, weight_dir: str):
        use_4bit = BNB_AVAILABLE and PEFT_AVAILABLE and torch.cuda.is_available()

        if use_4bit:
            print("    模式: 4-bit NF4 量化 + LoRA（GPU）")
            ...
            for path in [weight_dir, BERT_BASE, BERT_BASE.replace("google-bert/", "")]:
                try:
                    ...
                    print(f"    ✅ 4-bit 量化加载成功，来源: {path}")
                    break
                except Exception as e:
                    print(f"    ⚠️  {path}: {e}")
            if bert is None:
                raise RuntimeError("4-bit 量化加载失败，请检查 GPU 和 bitsandbytes 版本")
            bert = prepare_model_for_kbit_training(
                bert, use_gradient_checkpointing=True)
        else:
            print("    模式: 普通加载 + LoRA + INT8 量化（CPU）")
            bert = None
            for path in [weight_dir, BERT_BASE, BERT_BASE.replace("google-bert/", "")]:
                try:
                    bert = BertModel.from_pretrained(path, ignore_mismatched_sizes=True)
                    print(f"    ✅ BERT 加载成功，来源: {path}")
                    break
                except Exception:
                    continue
            if bert is None:
                raise RuntimeError("BERT 加载失败，请检查网络连接或本地模型路径")

        # 添加 LoRA
        if PEFT_AVAILABLE:
            all_module_names = [n for n, _ in bert.named_modules()]
            valid_targets = [
                t for t in LORA_TARGET
                if any(t in name for name in all_module_names)
            ]
            if not valid_targets:
                valid_targets = ["query", "value"]
                print(f"    退化到默认 target: {valid_targets}")
            else:
                skipped = set(LORA_TARGET) - set(valid_targets)
                for s in skipped:
                    print(f"    ⚠️  跳过不存在的模块: {s}")
            ...
            print(f"    ✅ LoRA 添加完成 (r={LORA_R}, target={valid_targets})")
            print(f"    LoRA 可训练: {trainable:,}/{total:,} ({100 * trainable / total:.2f}%)")

            if not use_4bit:
                # CPU 路径：LoRA 加完后再 INT8 量化（压缩非 LoRA 层体积）
                bert = torch.quantization.quantize_dynamic(
                    bert, {nn.Linear}, dtype=torch.qint8, inplace=False)
                print("    ✅ INT8 动态量化完成")
        else:
            print("    ⚠️  PEFT 不可用，仅 INT8 量化")
            if not use_4bit:
                bert = torch.quantization.quantize_dynamic(
                    bert, {nn.Linear}, dtype=torch.qint8, inplace=False)
    ...
    # ── 核心修复：安全保存/恢复（绕开 quant_state 问题）────
    def get_saveable_state(self) -> dict:
        """
        只保存 LoRA 参数 + MoE 头参数 的普通 float tensor。
        不碰量化相关的任何 buffer（quant_state/absmax/quant_map 等）。
        """
        state = {}
        # LoRA 参数（requires_grad=True 且名称含 lora_）
    ...
    def load_saveable_state(self, state: dict) -> None:
        """
        精确恢复 LoRA 参数 + MoE 头参数，不触碰 BERT 量化结构。
        """
        # 恢复 LoRA 参数
        ...
        for key, val in state.items():
            ...
            if pname in bert_named and bert_named[pname].requires_grad:
                try:
                    ...
                    print(f"    ⚠️  恢复 LoRA 参数失败 {pname}: {e}")
    ...
    def print_params(self) -> None:
        ...
        print(f"  ├─ LoRA 参数:   {lora_p:>12,}")
        print(f"  └─ MoE 头参数:  {moe_p:>12,}")
        print(f"  预估 fp16 大小: ~{total * 2 / 1024 / 1024:.1f} MB（未含量化压缩）")
...
def detect_attack_ids(model, tokenizer, test_data):
    ...
    print("\n" + "=" * 60)
    print("       异常攻击 CAN ID 检测结果（消融版：无蒸馏）")
    print("=" * 60)
    total_ids = set()
    for at, cids in sorted(attack_ids.items()):
        print(f"\n【{at.upper()} 攻击】CAN ID 数量: {len(cids)}")
        print(f"  {sorted(cids)[:20]}{'...' if len(cids) > 20 else ''}")
        total_ids |= cids
    print(f"\n【总计】{len(total_ids)} 个异常 CAN ID")
...
# ================================================================
def save_student(model, tokenizer, path: str):
    os.makedirs(path, exist_ok=True)
    print(f"\n💾 保存消融版模型 (fp16) → {path}")

    fp16_state = {}

    # 尝试合并 LoRA 权重到 BERT（使权重可独立推理）
    merged_bert = None
    try:
        if hasattr(model.bert, "merge_and_unload"):
            merged_bert = model.bert.merge_and_unload()
            print("  ✅ LoRA 权重已合并到 BERT")
            for n, p in merged_bert.named_parameters():
                try:
                    fp16_state[f"bert_merged.{n}"] = p.detach().cpu().to(torch.float16)
                except Exception:
                    pass
    except Exception as e:
        print(f"  ⚠️  LoRA 合并失败 ({e})，保存 LoRA 参数")
    ...
    if merged_bert is None:
        for n, p in model.bert.named_parameters():
            if "lora_" in n:
                try:
                    fp16_state[f"bert_lora.{n}"] = p.detach().cpu().to(torch.float16)
                except Exception:
                    pass
    ...
    for attr in ["gate", "expert_dos", "expert_fuzzy", "expert_gear"]:
        ...
        for pname, param in module.named_parameters():
            try:
                fp16_state[f"{attr}.{pname}"] = param.detach().cpu().to(torch.float16)
            except Exception:
    ...
    # 保存 meta（消融版：distill=null，ablation="no_distillation"）
    with open(os.path.join(path, "meta.json"), "w", encoding="utf-8") as f:
        json.dump({
            ...
            "bert_hidden":   768,
            "qlora": {
                "quantization": ("4bit-NF4" if (BNB_AVAILABLE and torch.cuda.is_available())
                                 else "INT8-dynamic"),
                "lora_r":      LORA_R,
                "lora_alpha":  LORA_ALPHA,
    ...
    flag = "✅ ≤50 MB" if tot_mb <= 50 else "❌ >50 MB"
    print(f"  最终大小: {tot_mb:.2f} MB  {flag}")
    return bin_mb, tot_mb


# ================================================================
# 10. 主流程（消融版：3 步，无教师，无 KL）
# ================================================================
def main():
    # ── 运行时初始化 ─────────────────────────────────────────
    set_seed(SEED)
    quant_mode = ("4-bit NF4 (GPU)" if (BNB_AVAILABLE and torch.cuda.is_available())
                  else "INT8 动态量化 (CPU)")
    print(f"运行设备: {DEVICE}")
    print(f"QLoRA 模式: {quant_mode}")
    print(f"bitsandbytes: {'✅ 可用' if BNB_AVAILABLE else '⚠️  不可用，使用 INT8 降级'}")
    print(f"PEFT/LoRA:    {'✅ 可用' if PEFT_AVAILABLE else '⚠️  不可用'}")

    print("=" * 65)
    print("  CAN 攻击检测 — 消融实验（QLoRA + MoE，去除蒸馏）")
    print("  实验条件：纯 CE 监督训练，无教师模型，无 KL 软标签")
    print("=" * 65)
    ...
    print("  Step 2/3  数据准备")
    print("=" * 55)
    # num_workers=0 确保在 Windows / PyCharm 下无多进程冲突
    ...
    # ── Step 3: 纯 CE 训练（无教师、无 KL、无软标签）────────
    print("\n" + "=" * 55)
    print("  Step 3/3  纯 CE 监督训练（消融：无蒸馏）")
    print("=" * 55)
    ...
    if not trainable_params:
        print("  ⚠️  无可训练参数，自动解冻 MoE 头...")
        for n, p in student.named_parameters():
            if any(x in n for x in
                   ["gate", "expert_dos", "expert_fuzzy", "expert_gear"]):
                p.requires_grad = True
        trainable_params = [p for p in student.parameters() if p.requires_grad]
        print(f"  已解冻 MoE 头: {sum(p.numel() for p in trainable_params):,} 参数")
    ...
    print(f"\n🚀 开始 CE 训练 ({STUDENT_EPOCHS} epochs)...")
    epoch_times, epoch_accs = [], []
    best_acc   = 0.0
    best_state = None  # 只存 LoRA + MoE 的普通 tensor 快照
    ...
    for epoch in range(STUDENT_EPOCHS):
        ...
        if steps > 0:
            ...
            if acc > best_acc:
                best_acc   = acc
                # ★ 只保存普通 tensor 快照，完全避免 quant_state key 冲突
    ...
    # 推理速度测量（热身 3 个 batch 后统计 100 个 batch）
    print("⏱️  测量推理速度（热身 3 批 + 统计 100 批）...")
    student.eval()
    ms_list, warmup = [], 0
    with torch.no_grad():
        for bench in te_ld:
    ...
    print("=================================================================")
    print(f"  实验条件：纯 CE 监督训练，无教师模型，无 KL 软标签")
    print()
    print("  【模型架构（消融组）】")
    print(f"  BERT:         12 层 bert-base-chinese")
